...
parser.add_argument('--k', default=10, help='reg coefs', type=int)
parser.add_argument
...
def loss(reconstr_a, orig_a, orig_x, y_pred, y_cf, z_mu, z_logvar, alpha):
    ...
    size_loss = torch.sum(reconstr_a) * 0.001
...
def get_counterfactual(inputs, cf_explainer, fexp, clf_model, data, params, y_cf, beta=1, device='cpu'):
    num_nodes = params['num_nodes']

    x, edge_index, _ = inputs
    #('x', x.shape, 'edge_index' , edge_index.shape)

    fact_mask = explain_inference(clf_model, fexp, inputs, device='cpu', bias = 0.0, k=args.k)
    nonfact_mask = 1-fact_mask

    fact_mask_ind = fact_mask.astype(bool)
    nonfact_mask_ind = nonfact_mask.astype(bool)

    
    fact_subgraph = edge_index[:, fact_mask_ind]
    nonfact_subgraph = edge_index[:, nonfact_mask_ind]

    fact_subgraph_adjs = to_dense_adj(fact_subgraph, data.batch, max_num_nodes=num_nodes)
    nonfact_subgraph_adjs = to_dense_adj(nonfact_subgraph, data.batch, max_num_nodes=num_nodes)


    inputs = x, edge_index, torch.tensor(fact_mask, dtype=torch.float32) #x : batchsize x 25, 10   edge_index: 2x Einbatch

    cf_of_factual_mask, z_mu, z_logvar = cf_explainer(inputs, beta=beta, y_cf=y_cf, batch=data.batch)
    # reconstr mask: batchsize, 25*25
    # x: batchsize*25  batchsize, 25
    batch_size = cf_of_factual_mask.shape[0]
    ...
    for i in range(batch_size):
        adj_matrix = cf_of_factual_mask[i].reshape(num_nodes, num_nodes)
        
        fact_subgraph_adj = fact_subgraph_adjs[i]
        nonfact_subgraph_adj = nonfact_subgraph_adjs[i]
        
        # Step 1: Identify nodes with degree > 0 in fact_subgraph_adj

        degree = fact_subgraph_adj.sum(dim=1)  # Sum over rows to get the degree of each node
        active_nodes = degree > 0  # Boolean mask for nodes with degree > 0

        # Step 2: Create a masking tensor
        active_mask = active_nodes.float().view(-1, 1) * active_nodes.float().view(1, -1)  # Create a mask for the adjacency matrix

        # Step 3: Apply the mask to the adjacency matrix to keep the original values
        adj_matrix = adj_matrix * active_mask  # This retains gradients

        adj_matrix = torch.max(adj_matrix, nonfact_subgraph_adj)

        assert(adj_matrix.shape == active_mask.shape)

        orig_adjs.append(orig_a[i,:, :])

        # adj_matrix = torch.max(nonfact_subgraph_adj[i], adj_matrix)
...
def test(cf_explainer, fexp, clf_model, loader, params, device ,save=False):
    cf_explainer.eval()
    total_loss_cf = 0
    correct = 0
    all_counterfactuals = []
    for data in loader:  # Iterate in batches over the training/test dataset.
        with torch.no_grad():
            ...
            y_cf = 1 - y_target
            reconstr_a, orig_a, y_pred, z_mu, z_logvar, all_batch_cfs = get_counterfactual((x, edge_index, None), cf_explainer, fexp, clf_model, data, params, y_cf, beta=1)
            if save:
    ...
    for i in range(len(all_counterfactuals)):
        visualize_cfs(all_counterfactuals[i][1], torch.bernoulli(all_counterfactuals[i][0]))
    if save:
...
def train(clf_model, fexp, cf_explainer, optimizer_cf, train_loader, val_loader, test_loader, device, args, params, temp=(5.0, 2.0)):
    ...
    for epoch in range(args.epochs):
        ...
        for data in train_loader:
            ...
            reconstr_a, orig_a, y_pred, z_mu, z_logvar, tmp = get_counterfactual((x, edge_index, None), cf_explainer, fexp, clf_model, data, params, y_cf, beta=1)
            alpha = 100
        ...
        cf_acc, test_reconstr, test_loss_kl, test_loss_cf = test(cf_explainer, fexp, clf_model, val_loader, params, device)
        print(f"Epoch {epoch + 1}/{args.epochs}, Loss: {loss_total.item()}, 'CF Acc': {cf_acc}, 'Recons loss: {test_reconstr}")


    cf_acc, test_reconstr, test_loss_kl, test_loss_cf = test(cf_explainer, fexp, clf_model, test_loader, params, device, save=True)
...
def run(args):
    ...
    fexp = torch.load(f'model/pretrained/explainers/fexpl_{dataset_name}.pth')
    fexp.eval()

    train(clf_model, fexp, cf_explainer, optimizer_cf, train_loader, val_loader, test_loader, device, args, params)
